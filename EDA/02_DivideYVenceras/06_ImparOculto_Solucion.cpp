...
int resolver(const std::vector<int>& vec, int ini, int fin) {
    if (ini != fin) {
        ...
        if (fin == ini + 1) {
            if (vec[ini] % 2 != 0) return vec[ini];
            else return vec[fin];
        }

        else if (vec[fin] != vec[mid] + (fin - mid) * 2) { //El elemento menor está a la derecha
            return resolver(vec, mid, fin);
        }

        else if (vec[ini] != vec[mid] - (mid - fin) * 2) { //El elemento mayor está a la izquierda
