...
void resolver(List<int> &lista, const int &frecuencia) {
	auto it = lista.begin();
	while (lista.size() > 1) {
		for (int k = 0; k < frecuencia; k++) {
			//std::cout << "Estoy en " << it.elem() << ". Mi contador es " << k << "\n";
			++it;
			if (it == lista.end()) it = lista.begin();
		}
		//std::cout << "Borrando " << it.elem() << ".\n";
		it = lista.erase(it);
		if (it == lista.end()) it = lista.begin();
	}
}
...
bool resuelveCaso() {
	...
	int nAlumnos, frecuencia;
	std::cin >> nAlumnos >> frecuencia;
    if (nAlumnos == 0 && frecuencia == 0)
        return false;

	List<int> alumnos = List<int>();
	for (int k = 1; k <= nAlumnos; k++) {
		alumnos.push_back(k);
	}

	resolver(alumnos, frecuencia);
