...
public:
	...
	std::pair<int, int> get_nNodos_nHojas(){
		if (_ra == NULL) return std::pair<int, int>(0, 0);
		std::pair<int, int> result = { 0, 0 };
		get_nNodos_nHojas_Aux(_ra, result);
	...
	int getNiveles() {
		struct recorridoProfundidad {
			Nodo* nodo;
			int profundidad;
			recorridoProfundidad(Nodo* n, int p) : nodo(n), profundidad(p) {};
		};
		int profundidad = 0;

		Queue<recorridoProfundidad> q;
		q.push_back({ _ra, 1 });
		if (_ra != NULL) {
			while (!q.empty())
			{
				recorridoProfundidad front = q.front();
				q.pop_front();
				if (front.nodo->_iz != NULL) q.push_back({ front.nodo->_iz, front.profundidad + 1 });
				if (front.nodo->_dr != NULL) q.push_back({front.nodo->_dr, front.profundidad + 1});

				if (front.profundidad > profundidad) profundidad = front.profundidad;
			}
		}
...
private:
	...
	void get_nNodos_nHojas_Aux(Nodo* n, std::pair<int, int>& result) {
		result.first++;
		bool hijoIzq = (n->_iz != NULL), hijoDer = (n->_dr != NULL);

		if (!hijoIzq && !hijoDer) result.second++;

		if (hijoIzq) get_nNodos_nHojas_Aux(n->_iz, result);
		if (hijoDer) get_nNodos_nHojas_Aux(n->_dr, result);
	}
...
Arbin<int> construyeArbol() {
	...
	else if (raiz == '.') Arbin<int>();
	return Arbin<int>();
}

// función que resuelve el problema
void resolver(Arbin<int> &tree) {
	std::pair<int, int> result = tree.get_nNodos_nHojas();

	std::cout << result.first << " " << result.second << " " << tree.getNiveles() << "\n";
}
