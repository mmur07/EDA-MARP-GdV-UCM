// EDA_ReconstruccionArboles.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int encuentraElem(const std::vector<int>& vec, int elem) {
	for (int k = 0; k < vec.size(); k++) {
		if (elem == vec[k]) return k;
	}
...
template < typename T >
Arbin <T> reconstruye_rec(int limIzq, int limDer, int k, const vector <T>& inorden, const vector <T>& postorden) {
	if (k < 0) return Arbin<T>();

	int raiz = postorden[k];
	int posInorden = encuentraElem(inorden, raiz);

	if (posInorden >= limIzq && posInorden <= limDer) {

		if (limIzq == limDer - 1) return Arbin<T>(raiz);

		Arbin<T> derecha = reconstruye_rec(posInorden, limDer, k - 1, inorden, postorden);
		Arbin<T> izquierda = reconstruye_rec(limIzq, posInorden, k - 1, inorden, postorden);

		return Arbin<T>(izquierda, raiz, derecha);
	}
...
template < typename T >
Arbin <T> reconstruye(const vector <T >& inorden, const vector <T >& postorden) {
	int max_Altura = 0;

	Arbin<T> tree = reconstruye_rec(0, postorden.size(), postorden.size() - 1, inorden, postorden);

	return tree;
}

// función que resuelve el problema
int resolver(std::vector<int> inOrden, std::vector<int> postOrden) {
	Arbin<int> tree = reconstruye(inOrden, postOrden);
	return tree.talla();
}
...
bool resuelveCaso() {
	...
	if (nNodos == -1)
		return false;
	std::vector<int> inOrden, postOrden;
	int aux;
	...
	std::cout << resolver(inOrden, postOrden) << "\n";
	// escribir sol
