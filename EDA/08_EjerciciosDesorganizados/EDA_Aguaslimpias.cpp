...
private:
	...
	static bool comparaAux(Nodo* r1, Nodo* r2) {
		if (r1 == r2)
			...
			// En el if anterior nos aseguramos def
...
Arbin<int> construyeArbol() {
	int raiz = 0;
	std::cin >> raiz;
	if (raiz != -1) {
		Arbin<int> izq = construyeArbol();
		Arbin<int> der = construyeArbol();
		return Arbin<int>(izq, raiz, der);
	}
	else return Arbin<int>();
}

int caudalAux(Arbin<int> tree, int &caudalesNavegables) {
	if (tree.esVacio()) return 0;
	int caudalDerecho = caudalAux(tree.hijoDr(), caudalesNavegables);
	int caudalIzquierdo = caudalAux(tree.hijoIz(), caudalesNavegables);

	bool hijosVacios;
	hijosVacios = (!tree.hijoDr().esVacio() && !tree.hijoIz().esVacio());

	if (caudalDerecho >= 3) {
		caudalesNavegables++;
	}
	if (caudalIzquierdo >= 3) {
		caudalesNavegables++;
	}
	if (caudalDerecho == 0 && caudalIzquierdo == 0 && !hijosVacios) {
		if (tree.raiz() > 0) {
			return 0;
		}
		else return 1;
		...
		//std::cout << "Caudal izquierdo: " << caudalIzquierdo << ", Caudal derecho: " << caudalDerecho << ", Embalse: " << tree.raiz() << ". RESULTADO: " << caudalDerecho + caudalIzquierdo - tree.raiz() << "\n"; //Debugeo
		if (caudalDerecho + caudalIzquierdo - tree.raiz() < 0) {
			//std::cout << "Devolvi 0\n";
			return 0;
		}
		else return (caudalDerecho + caudalIzquierdo - tree.raiz());
	}	
}

int getCaudal(const Arbin<int>& tree) {
	int caudalesNavegables = 0;
	caudalAux(tree, caudalesNavegables);
	return caudalesNavegables;
}

// función que resuelve el problema
int resolver(const Arbin<int> &tree) {

	return getCaudal(tree);
}
...
void resuelveCaso() {
	...
	Arbin<int> tree = construyeArbol();

    std::cout << resolver(tree) << "\n";
    // escribir sol
