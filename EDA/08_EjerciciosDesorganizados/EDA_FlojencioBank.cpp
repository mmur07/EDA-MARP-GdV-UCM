...
bool resuelveCaso() {
	...
	HashMap<string, int> registroBancario;
	string numTarjeta;
	int cargo;
	for (int k = 0; k < nOperaciones; k++) {
		std::cin >> numTarjeta >> cargo;

		auto it = registroBancario.find(numTarjeta);
		if (it == registroBancario.end()) {
			registroBancario.insert(numTarjeta, cargo);
		}
		else {
			it.value() += cargo;
		}
