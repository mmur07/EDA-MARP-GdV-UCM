...
int resolver_aux(const Arbin<int>& tree, int& maxDiametro) {
	if (tree.esVacio()) return 0;

	int alturaIzquierda = resolver_aux(tree.hijoIz(), maxDiametro);
	int alturaDerecha = resolver_aux(tree.hijoDr(), maxDiametro);

	if (alturaDerecha + alturaIzquierda + 1 > maxDiametro) maxDiametro = alturaDerecha + alturaIzquierda + 1;

	return std::max<int>(alturaDerecha, alturaIzquierda) + 1;
}
...
int resolver(const Arbin<int> &tree) {
	...
	resolver_aux(tree, max_Diametro);
...
Arbin<int> construyeArbol() {
	...
	if (raiz == '*') {
		...
		return Arbin<int>(izq, 1, der);
	}
	else if (raiz == '.') return Arbin<int>();
}
