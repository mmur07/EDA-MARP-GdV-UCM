...
int getGeneraciones(const Arbin<int>& arbol, int currentGeneration, bool &valid) {
	if (valid && !arbol.esVacio()) {
		int generacionIzquierdo = 0, generacionDerecho = 0;

		currentGeneration++;
		if (!arbol.hijoIz().esVacio()) {
			if (arbol.hijoIz().raiz() <= arbol.raiz() - 18) {
				generacionIzquierdo = getGeneraciones(arbol.hijoIz(), currentGeneration, valid);

				if (!arbol.hijoDr().esVacio()) {
					if (arbol.hijoDr().raiz() <= arbol.hijoIz().raiz() - 2) {
						generacionDerecho = getGeneraciones(arbol.hijoDr(), currentGeneration, valid);
					}
					else {
						valid = false;
						return -1;
				...
				return std::max<int>(generacionDerecho, std::max<int>(generacionIzquierdo, currentGeneration));
			}
			else valid = false;
			return -1;
		}
		...
		else return currentGeneration;
	}
	else return currentGeneration;
}

// función que resuelve el problema
int resolver(const Arbin<int> &arbol) {
	bool valid = true;
	int generaciones = getGeneraciones(arbol, 0, valid);
	if (valid) return generaciones;
	else return -1;
}
...
void resuelveCaso() {
	...
	int resultado = resolver(tree);
	if (resultado != -1) std::cout << "SI " << resultado << "\n";
	else std::cout << "NO\n";
    // escribir sol
