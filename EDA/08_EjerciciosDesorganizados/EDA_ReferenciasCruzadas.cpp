...
void addWord(string word, int currentLine, TreeMap<string, std::vector<int>>& mapa) {
	if (word.length() > 2) {
		auto it = mapa.find(word);
		if (it == mapa.end()) mapa.insert(word, { currentLine + 1});
		else {
			if (it.value()[it.value().size() - 1] != currentLine + 1) {
				it.value().push_back(currentLine + 1);
			}
		}
	}
}
...
bool resuelveCaso() {
	...
	TreeMap<string, std::vector<int>> mapa;
	for (int k = 0; k < nLineas; k++) {
		while (linesRead < nLineas) {
			newChar = std::tolower(std::cin.get());
			if (newChar == ' ') {
				addWord(newWord, linesRead, mapa);
				newWord = "";
			}
			else if (newChar == '\n') {
				addWord(newWord, linesRead, mapa);
