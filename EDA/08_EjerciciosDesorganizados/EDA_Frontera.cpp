...
void encuentraFrontera (Arbin<int> tree, std::vector<int> &frontera) {
	bool izVacio = tree.hijoIz().esVacio(), derVacio = tree.hijoDr().esVacio();
	if (!izVacio) encuentraFrontera(tree.hijoIz(), frontera);
	if (!derVacio) encuentraFrontera(tree.hijoDr(), frontera);
	else if (izVacio && derVacio) frontera.push_back(tree.raiz());
}
...
void resolver(Arbin<int> tree) {
	if (!tree.esVacio()) {
		...
		encuentraFrontera(tree, frontera);
