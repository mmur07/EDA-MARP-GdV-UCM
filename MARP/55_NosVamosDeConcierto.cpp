...
int getMaxGrupos(const std::vector<Festival>& festivales, int i, int j, Matriz<int> &solucion) {
    if (solucion[i][j] == -1) {
        if (i == 0 || j == 0) solucion[i][j] = 0;
        else if (festivales[i - 1].precio > j) solucion[i][j] = getMaxGrupos(festivales, i - 1, j, solucion);
        else solucion[i][j] = std::max(getMaxGrupos(festivales, i - 1, j, solucion), getMaxGrupos(festivales, i - 1, j - festivales[i - 1].precio, solucion) + festivales[i - 1].numGrupos);
    }
...
bool resuelveCaso() {
    ...
    int presupuesto, nGrupos, lec1, lec2;
    std::cin >> presupuesto >> nGrupos;
    ...
    for (int k = 0; k < nGrupos; k++) {
        std::cin >> lec1 >> lec2;
        festivales.push_back({ lec1, lec2 });
    }
    Matriz<int> matriz(nGrupos + 1, presupuesto + 1, -1);

    getMaxGrupos(festivales, nGrupos, presupuesto, matriz);

    std::cout << matriz[nGrupos][presupuesto] << "\n";
