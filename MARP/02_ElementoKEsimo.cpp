...
public:
    ...
    int kesimo(int k) {
        return kesimo_rec(raiz, k);
    }

    int kesimo_rec(Link a, int k) {
        if (a == nullptr) return -1;
        if (a->nHijosIzq == k) return a->elem;
        else if (k < a->nHijosIzq) return kesimo_rec(a->iz, k);
        else return kesimo_rec(a->dr, k - a->nHijosIzq);
    }
...
protected:
    ...
    void rotaIzq(Link& r1) {
        //r1->nHijosIzq += r1->dr->nHijosIzq - 1;
        r1->dr->nHijosIzq = r1->dr->nHijosIzq + r1->nHijosIzq;
        Link r2 = r1->dr;
        r1->dr = r2->iz;
        r2->iz = r1;
        r1->altura = std::max(altura(r1->iz), altura(r1->dr)) + 1;
...
bool resuelveCaso() {
    ...
    for (int k = 0; k < nConsultas; k++) {
        std::cin >> aux;
        int kesimo = avl.kesimo(aux);
        if (kesimo != -1) std::cout << kesimo << "\n";
        else std::cout << "??\n";
    }
    std::cout << "---\n";
    return true;
